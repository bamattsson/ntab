...
class PredictDataset(Dataset):
    """Dataset of (fingerprint, props, target_index, standard_type_index, name, uniprot_id) tuples.

    Used for inference — no labels or assay_ids.

    Args:
        fps_matrix: Full fingerprint matrix, shape (N_unique, fp_size), float32 tensor.
        props_matrix: Full mol properties matrix, shape (N_unique, n_props), float32 tensor.
        fp_indices: Row index per sample into fps_matrix and props_matrix, length N.
        target_indices: Integer target index per sample, length N.
        standard_type_indices: Integer standard type index per sample, length N.
        names: Molecule name per sample, length N.
        uniprot_ids: Uniprot ID per sample, length N.
    """
    ...
    def __init__(
        ...
        self._uniprot_ids = list(uniprot_ids)
